...
const departmentEmails = {
  ...
  Electricity: "electricity-dept@example.com",
  Sanitation: "sanitation-dept@example.com",
  "Road Maintenance": "mayurshenoy02@gmail.com",
  "Public Transport": "transport-dept@example.com",
  "Health Services": "health-dept@example.com",
...
app.post("/api/feedback", async (req, res) => {
  try {
    ...
    // Save feedback in separate model (for admin viewing)
    ...
    // Link feedback to complaint for completeness
    complaint.feedback = { rating, comments, date: new Date() };
...
app.get("/api/feedbacks", async (req, res) => {
  try {
    // Populate related complaint info for admin
    const feedbacks = await Feedback.find()
      .sort({ createdAt: -1 })
      .lean();

    const complaints = await Complaint.find({}, { complaintId: 1, name: 1, email: 1 }).lean();
    const complaintMap = Object.fromEntries(
      complaints.map((c) => [c.complaintId, c])
    );
    ...
    const formatted = feedbacks.map((f) => ({
      ...
      rating: f.rating,
      comment: f.comment,
      name: complaintMap[f.complaintId]?.name || "Unknown",
      email: complaintMap[f.complaintId]?.email || "N/A",
...
app.get("/api/complaints/:id/report", async (req, res) => {
  try {
    ...
    doc.fontSize(22).fillColor("#0D47A1").text("Citizen Complaint Report", {
      align: "center",
      underline: true,
    });
    doc.moveDown(1);
    ...
    doc
      ...
      .fillColor("#616161")
      .text(
        "This report was automatically generated by the Citizen Complaint Management System.",
        { align: "center" }
      );
